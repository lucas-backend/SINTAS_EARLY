...
const scanAt = (minutes, seconds = 0) =>
...
describe("attendance domain", () => {
  ...
  it("rejects a session whose end is not after its start", () => {
    expect(() =>
      attendanceSessionSchema.parse({ startAt, endAt: startAt }),
    ).toThrow();
    expect(() =>
      attendanceSessionSchema.parse({
        startAt,
        endAt: new Date("2026-09-17T07:59:59.000Z"),
      }),
    ).toThrow();
  });
  ...
  it("accepts the exact opening boundary at 15 minutes before start", () => {
    expect(
      classifyAttendanceScan({ startAt, endAt, scanAt: scanAt(-15) }),
    ).toEqual({
  ...
  it("accepts a scan at the session start as present", () => {
    expect(
      classifyAttendanceScan({ startAt, endAt, scanAt: scanAt(0) }),
    ).toEqual({
      status: AttendanceStatus.HADIR,
      lateMinutes: 0,
    });
  ...
  it("keeps a scan just before and exactly at 15 minutes as present", () => {
    expect(
      classifyAttendanceScan({ startAt, endAt, scanAt: scanAt(14, 59) }),
    ).toEqual({
      status: AttendanceStatus.HADIR,
      lateMinutes: 0,
    });
  ...
  it("marks a scan after 15 minutes as late using the server scan time", () => {
    expect(
      classifyAttendanceScan({ startAt, endAt, scanAt: scanAt(16, 30) }),
    ).toEqual({
  ...
  it("rejects scans before the window and after the session", () => {
    expect(() =>
      classifyAttendanceScan({ startAt, endAt, scanAt: scanAt(-15, -1) }),
    ).toThrow(RangeError);
    expect(() =>
      classifyAttendanceScan({
        startAt,
        endAt,
        scanAt: new Date("2026-09-17T09:00:00.001Z"),
      }),
    ).toThrow(RangeError);
  });
});
