...
describe('health routes', () => {
  it('reports a live process without database access', async () => {
    ...
    const response = await request(createApp({ prisma, logger })).get('/health/live')
  ...
  it('reports readiness only when the database responds', async () => {
    ...
    const response = await request(createApp({ prisma, logger })).get('/health/ready')
  ...
  it('returns the consistent error shape for unknown routes', async () => {
    const response = await request(createApp({ prisma: {}, logger })).get('/missing')
