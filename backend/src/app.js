...
export function createApp({ prisma = defaultPrisma, logger, env: configuredEnv } = {}) {
  const app = express()
  ...
  const rateLimitOptions = {
    ...
    legacyHeaders: false,
    handler: (_req, _res, next) => next(new AppError(429, 'RATE_LIMITED', 'Terlalu banyak percobaan. Silakan coba lagi nanti.')),
  }
  app.use('/api/v1/auth/login', rateLimit({ ...rateLimitOptions, limit: 10 }))
  app.use('/api/v1/auth/forgot-password', rateLimit({ ...rateLimitOptions, limit: 5 }))
